...
function footer() {
  return `<footer class="site-footer"><div><a class="footer-name" href="/">NightLemon</a><p>Small tools. Real life.</p></div><div class="footer-links"><a href="/directory/">项目索引</a><a href="/nado-card/redemptions.json">活动订阅源</a><a href="https://github.com/NightLemon" target="_blank" rel="noopener noreferrer">GitHub ${icon("external")}</a></div><span class="copyright">© ${new Date().getUTCFullYear()} NightLemon</span></footer>`;
}
...
function home() {
  ...
  return document(
    "NightLemon — 把生活里的小麻烦，做成顺手的工具。",
    "航空、生活工具与个人 Agent。NightLemon 的个人作品集。",
    "/",
    `${header(true)}<main id="main"><section class="hero wrap" aria-labelledby="hero-title"><div class="hero-copy"><p class="eyebrow"><span class="little-spark">✳</span> AN INDEPENDENT COLLECTION</p><h1 id="hero-title">把生活里的小麻烦，<br/>做成<span class="lemon-underline">顺手的工具。</span></h1><p class="hero-description">从一架飞机的注册号，到一张信用卡的到期提醒。<br class="desktop-break"/>一些源于好奇、服务日常，也在不断生长的个人作品。</p><a class="text-button" href="#work">看看我的作品 ${icon("arrow")}</a></div><div class="hero-art" aria-hidden="true"><div class="orbit orbit-one"></div><div class="orbit orbit-two"></div><span class="orbit-dot"></span><img src="/assets/lemon.svg" width="172" height="172" alt=""/><span class="hero-art-caption">SMALL TOOLS<br/><span>REAL LIFE.</span></span><span class="art-note">a little curiosity goes a long way</span></div></section><section class="work-section wrap" id="work" aria-labelledby="work-title"><div class="section-heading"><div><p class="eyebrow">SELECTED WORK / 01—04</p><h2 id="work-title">从自己的需求出发<span class="accent-dot">.</span></h2></div><p>有些已经能用，有些还在生长。<br/>每一个，都有一个具体的起点。</p></div><div class="project-grid">${featured.map(projectCard).join("")}</div></section><section class="more-section wrap" aria-labelledby="more-title"><div class="section-heading compact-heading"><div><p class="eyebrow">A FEW MORE THINGS</p><h2 id="more-title">好奇心的其他去处<span class="accent-dot">.</span></h2></div><span class="section-note">航空 / 影像 / 记忆</span></div><div class="more-grid">${more.map((p, i) => `<a class="small-project" href="/projects/${p.id}/">${cover(p, i + 4, true)}<div class="small-project-title"><h3>${esc(p.name)}</h3>${icon("external")}</div><p>${esc(p.summary)}</p></a>`).join("")}</div></section><section class="about-section wrap" id="about" aria-labelledby="about-title"><div class="about-mark" aria-hidden="true"><img src="/assets/lemon.svg" width="86" height="86" alt=""/><span>HELLO THERE.</span></div><div class="about-copy"><p class="eyebrow">A LITTLE ABOUT ME</p><h2 id="about-title">你好，我是 NightLemon。</h2><p>我喜欢把具体的问题做成小工具。航空、旅行、日常事务，还有个人 Agent，都是我愿意继续探索的方向。</p><p>我关心信息有没有讲清楚，操作能不能更顺手，以及一个工具怎样慢慢长成自己想要的样子。</p><a class="text-button" href="https://github.com/NightLemon" target="_blank" rel="noopener noreferrer">在 GitHub 找到我 ${icon("external")}</a></div></section></main>${footer()}`,
    "home",
  );
}
function detail(p) {
  ...
  return document(
    ...
    p.summary,
    `/projects/${p.id}/`,
    `${header()}<main id="main" class="case-main wrap"><a class="back-link" href="/#work">${icon("arrow")} 返回作品集</a><section class="case-intro"><div><p class="eyebrow">${esc(p.eyebrow)} / ${String(index + 1).padStart(2, "0")}</p><h1>${esc(p.name)}</h1><p class="case-headline">${esc(p.headline)}</p></div><div class="case-actions"><span class="status-label"><i class="status-dot ${p.repo ? "" : "status-private"}"></i>${esc(p.status)} · ${esc(p.access)}</span><div class="action-row">${p.live ? `<a class="button button-dark" href="${esc(p.live)}" target="_blank" rel="noopener noreferrer">打开${p.id === "fund-dashboard" ? "看板" : p.id === "aircraft-finder" ? "查机" : "工具"} ${icon("external")}</a>` : ""}${p.repo ? `<a class="button button-outline" href="${esc(p.repo)}" target="_blank" rel="noopener noreferrer">查看源码 ${icon("github")}</a>` : '<span class="private-note">通过下面的案例了解这个作品</span>'}</div></div></section><figure class="case-figure"><a class="case-image-button" href="${esc(p.image)}" data-lightbox-src="${esc(p.image)}" data-caption="${esc(p.imageCaption)}" aria-label="放大${esc(p.name)}的展示图">${cover(p, index)}<span class="zoom-label">查看大图 ${icon("external")}</span></a><figcaption>${esc(p.imageCaption)}${["flight-map", "photo-map"].includes(p.id) ? ' <a href="https://www.openstreetmap.org/copyright" target="_blank" rel="noopener noreferrer">© OpenStreetMap contributors</a>' : ""}</figcaption></figure><section class="case-story" aria-labelledby="story-title"><div class="story-main"><p class="eyebrow">THE IDEA</p><h2 id="story-title">一个具体的起点。</h2><p class="story-lead">${esc(p.description)}</p>${p.sections.map((s, i) => `<section class="story-section"><span class="story-number">0${i + 1}</span><div><h3>${esc(s.title)}</h3><p>${esc(s.body)}</p></div></section>`).join("")}</div><aside class="case-facts" aria-label="项目资料"><p class="eyebrow">PROJECT NOTES</p><dl>${p.facts.map((f) => `<div><dt>${esc(f.label)}</dt><dd>${esc(f.value)}</dd></div>`).join("")}<div><dt>代码</dt><dd>${p.repo ? "公开源码" : "私人项目"}</dd></div></dl><div class="boundary"><h3>使用之前</h3><p>${esc(p.boundary)}</p></div></aside></section><a class="next-project" href="/projects/${next.id}/"><div><p class="eyebrow">NEXT PROJECT</p><h2>${esc(next.name)}</h2><p>${esc(next.headline)}</p></div><span class="next-arrow">${icon("arrow")}</span></a></main>${footer()}<dialog class="image-dialog" aria-label="项目展示图"><button class="dialog-close" type="button" aria-label="关闭大图">×</button><figure><img alt=""/><figcaption></figcaption></figure></dialog>`,
    "case-page",
  );
}
function directory() {
...
const routes = [
  ...
  [
    ...
    document(
      ...
      "/404.html",
      `${header()}<main id="main" class="not-found wrap"><p class="eyebrow">404 / A SMALL DETOUR</p><h1>走到了一条小岔路。</h1><p>这个地址暂时没有内容，回作品集继续逛逛吧。</p><a class="button button-dark" href="/">回到首页 ${icon("arrow")}</a></main>${footer()}`,
    ),
  ],
];
